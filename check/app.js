...
function addBoxes(row, column) {
  let plateColorSrc = ["img/purple_square.svg"];
...
$(".box_svg").click(function () {
  ...
  initBlockPos[0] = userAnswerpos[0];
  initBlockPos[1] = userAnswerpos[1];
});
...
function moveBlock(cordinatx, cordinaty, anime) {
  ...
  if (anime === true) {
    $("#moveBlock").animate(
      ...
      "easeOutQuart",
      function () {
        setTimeout(() => moveBlock(initBlockPos[0], initBlockPos[1], true), 0);
      }
    );
  } else {
    $("#moveBlock").css({
...
function randomArrows(directionArr) {
  for (let i = 0; i < directionArr.length; i++) {
    switch (directionArr[i]) {
      case "l":
        moveBlock((blockPos[0] -= 1), blockPos[1], true);
      ...
      case "r":
        ...
        break;
      case "t":
        moveBlock(blockPos[0], (blockPos[1] -= 1), true);
